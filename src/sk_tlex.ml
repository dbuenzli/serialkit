...
module Tloc = struct
  ...
  | true ->
      pf ppf "File \"%a\", line n/a, characters n/a"
        pp_path l.file
  | false ->
      let pp_lines ppf l = match (snd l.line_s) = (snd l.line_e) with
      | true -> pf ppf "line %d" (snd l.line_s)
      | false -> pf ppf "lines %d-%d" (snd l.line_s) (snd l.line_e)
      in
  ...
  | false ->
      let pp_lines ppf l =
        ...
        | true ->  pf ppf "%d.%d-%d" (snd l.line_s) col_s col_e
        | false ->
            pf ppf "%d.%d-%d.%d" (snd l.line_s) col_s (snd l.line_e) col_e
      in
